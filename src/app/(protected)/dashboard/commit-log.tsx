...
import React from 'react' // 0.9k (gzipped: 0.7k)
        ...
        <ul className='space-y-8'>
            {commits?.map((commit, commitIdx) => (
                <li key={commit.id} className='relative flex gap-x-4'>
                    <div className={cn(
                        commitIdx === commits.length - 1 ? 'h-0' : '-bottom-8',
                    ...
                    <div className='flex-auto rounded-md bg-white p-3 ring-1 ring-inset ring-gray-200'>
                        ...
                        <pre className='mt-2 whitespsace-pre-wrap text-sm leading-6 text-gray-500'>
                            {commit.summary}
                        </pre>
                    </div>
                </li>
